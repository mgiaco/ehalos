...
char* halos_prompt = "halos> ";
char* halos_space =  " ";
char* halos_except =  "COMMAND IGNORED: exceeded maximum command length";
char* help = "type --help after each command for info of usage";
...
int8_t display_help(int argc, char* argv[]){
	char* helpLine = "following commands are available:\r\n";
...
//	}
	...
	do{
		if(process_info!=NULL){
			...
			priority = process_info->sched_priority;
			//TODO CPU TIME CALC

			strcat(top_string,halos_itoa(pid));
			strcat(top_string," || ");
			//top_string = strcat(top_string,process_name);
			strcat(top_string," || ");
			strcat(top_string,halos_itoa(priority));
...
int8_t sendResponse1(int argc, char* argv[]){
	...
	char* response = "start not yet implemented\r\n";
...
int8_t load_commands(){
	...
	add_command("kill",sendResponse3,"tuat no nix");
...
int8_t interpret_command(char* command){
	...
	if(command_node != NULL){
		...
		set_color(TEXT);
		new_line();
		cmd_ret_val =command_node->execute(argc,parameters,&retval);
		if(cmd_ret_val < 0){
			return cmd_ret_val;
		}
	}else{
...
void console_draw_string(char* line){
	int i = 0;
	int str_len=0;
 ...
 *
 */
void draw_console_line(char* msg, int32_t color, int8_t type){
